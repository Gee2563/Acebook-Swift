...
struct ViewPost: View {
    var post: Post
    @State private var currentUserID = UserDefaults.standard.string(forKey: "currentUserID")
            ...
            VStack(alignment: .leading, spacing: 10) {
                if currentUserID == post.userId {
                    HStack {
                        ...
                        Spacer()
                        Button(action: {
                            deletePostByID(post._id) { error in
                                if let error = error {
                                    deleteError = error
                                    showAlert = true
                                } else {
                ...
                if isEditing {
                    ...
                    Button(action: {
                        updatePostByID(id: post._id, newContent: editedContent) { error in
                            if let error = error {
                                deleteError = error
                                showEditAlert = true
                            } else {
                        ...
                        Section{
                            ...
                            if !post.likes.isEmpty {
                                ...
                                if post.likes.contains(currentUserID) {
                                    Button(action: {
                                        updateLikesByID(id: post._id, userId: currentUserID) { error in
                                            if let error = error {
                                                deleteError = error
                                                showAlert = true
                                            } else {
...
//#Preview {
//    UserDefaults.standard.set("66d601a5bd717d3e9960b8a0", forKey: "currentUserID")
//    
////    return ViewPost(
